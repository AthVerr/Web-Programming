...
HashMap<String,User> hm = null;
   MySqlDataStoreUtilities sd = null;
	...
	protected void doPost(HttpServletRequest request,
		...
		if(user!=null)
		{
		 String user_password = user.getPassword();//get the user pass
		 if (password.equals(user_password)) //check if the password match the user pass
			{
	...
	@Override  
	protected void doGet(HttpServletRequest request,  //get request 
			HttpServletResponse response) throws ServletException, IOException {
		response.setContentType("text/html");
		PrintWriter pw = response.getWriter();
	...
	protected void displayLogin(HttpServletRequest request,
		...
		Utilities utility = new Utilities(request, pw); //connect with utilities class 
		utility.printHtml("Header.html");
		pw.print("<div class='post' style='float: none; width: 100%'>");
		pw.print("<h2 class='title meta'><a style='font-size: 24px;color:red;'>User Login</a></h2>"
				+ "<div class='entry'>"
				+ "<div style='width:400px; margin:25px; margin-left: auto;margin-right: auto;'>");
		if (error)
			pw.print("<h4 style='color:black'>Please check your username, password and user type!</h4>");
		HttpSession session = request.getSession(true);
		if(session.getAttribute("login_msg")!=null){			
			pw.print("<h4 style='color:black'>"+session.getAttribute("login_msg")+"</h4>");
			session.removeAttribute("login_msg");
		} //create  the html for lohin , header and everyhting else 
		pw.print("<form method='post' action='Login'>"
				...
				+ "</td></tr><tr><td></td><td>"
				+ "<strong><a class='' href='Registration' style='float: right;height: 20px margin: 20px;'>Not have an account? Register for free!</a></strong>"
				+ "</td></tr></table>"
				+ "</form>" + "</div></div></div>");
